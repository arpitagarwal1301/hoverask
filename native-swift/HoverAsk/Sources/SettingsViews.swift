...
struct SettingsSidebar: View {
    ...
    private var bottomAppCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            SettingsBottomAction(systemName: "heart", title: "Buy coffee", isDisabled: SupportConfig.supportURL == nil) {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            SettingsGroup {
                ...
                SettingsStatusRow(icon: "key", title: "BYOK Cloud", detail: "API keys will be stored in Keychain.", status: "Not configured", tone: .warning, actionSystemName: "pencil", actionLabel: "Open providers") {
                    selectedSection = .providers
                }
            }
...
struct AssistantSettingsPage: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            SettingsGroup(title: "Default behavior") {
                SettingsPickerRow(title: "Provider", detail: "Only ready CLI providers appear here.", selection: $settings.provider) {
    ...
    private var providerPickerOptions: [ProviderSelection] {
        ...
        for provider in [AssistantProvider.codex, .claude, .cursor, .opencode, .antigravity] {
            if let health = viewModel.providerHealth.first(where: { $0.provider == provider }), health.installed && health.authenticated {
                options.append(provider.selection)
            }
        }
        if options.count == 1 {
            options.append(contentsOf: [.codex, .claude])
        }
        return Array(NSOrderedSet(array: options).compactMap { $0 as? ProviderSelection })
    }
}
...
struct VoiceSettingsPage: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            SettingsPageHeader(title: "Voice", subtitle: "Control listening, dictation, and spoken replies.")
            SettingsGroup(title: "Input") {
                ...
                SettingsStatusRow(icon: "mic", title: "Microphone", detail: "Managed by macOS privacy settings.", status: "Allowed", tone: .success)
                SettingsStatusRow(icon: "waveform", title: "Speech recognition", detail: "Needed for live transcripts.", status: "Allowed", tone: .success)
            }
...
struct ProviderSettingsPage: View {
    ...
    var body: some View {
        ScrollView(showsIndicators: true) {
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top) {
                    SettingsPageHeader(title: "Providers", subtitle: "Connect accounts, choose models, and test where HoverAsk answers from.")
                    Spacer()
                    SettingsActionButton("Reset provider defaults", systemName: "arrow.clockwise") {
                        resetProviderDefaults()
                    }
                }
                routeBuilder
                providerBucket("Account CLIs", detail: "Use logged-in CLI accounts.", providers: AssistantProvider.cliProviders)
                providerBucket("Private Local", detail: "Run on this Mac or local server when available.", providers: AssistantProvider.localProviders)
                providerBucket("BYOK Cloud", detail: "API keys are stored only in macOS Keychain.", providers: AssistantProvider.byokProviders)
            }
            .padding(.horizontal, 28)
            .padding(.vertical, 24)
        }
        ...
        SettingsGroup(title: nil) {
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Default answer source")
                        .font(.system(size: 13, weight: .semibold, design: .rounded))
                    Text("Auto tries Account CLIs, then Private Local, then BYOK Cloud.")
                        .font(.system(size: 11, weight: .medium, design: .rounded))
                        .foregroundStyle(.white.opacity(0.50))
                }
                Spacer()
    ...
    private func providerBucket(_ title: String, detail: String, providers: [AssistantProvider]) -> some View {
        let readyCount = providers.filter { providerReady($0) }.count
        return SettingsBucket(title: title, status: "\(readyCount) ready", tone: readyCount > 0 ? .success : .warning, detail: detail) {
    ...
    private func providerRow(_ provider: AssistantProvider) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 12) {
                ...
                Circle()
                    .fill(providerReady(provider) ? SettingsStatusTone.success.color : SettingsStatusTone.warning.color)
                ...
                Spacer()
                SettingsSmallBadge(provider.category.title)
                Text(providerStatus(provider))
                    .font(.system(size: 12, weight: .semibold, design: .rounded))
                    .foregroundStyle(providerReady(provider) ? SettingsStatusTone.success.color : SettingsStatusTone.warning.color)
    ...
    private func providerActions(_ provider: AssistantProvider) -> some View {
        ...
        case .byok:
            if BYOKKeychain.hasKey(for: provider) {
                SettingsActionButton("Models") { viewModel.fetchModels(for: provider) }
                SettingsActionButton("Test") { viewModel.testProvider(provider) }
                SettingsActionButton("Delete") { confirmDeleteKey(provider) }
            } else {
    ...
    private func providerModelControls(_ provider: AssistantProvider) -> some View {
        ...
        case .codex:
            HStack {
                SettingsInlinePicker("Model", selection: $settings.codexModel) {
                    ForEach(CodexModel.allCases) { model in Text(model.title).tag(model) }
                }
                SettingsInlinePicker("Effort", selection: $settings.codexEffort) {
                    ForEach(CodexReasoningEffort.allCases) { effort in Text(effort.title).tag(effort) }
                }
            }
            .padding(.leading, 48)
        case .claude:
            HStack {
                SettingsInlinePicker("Model", selection: $settings.claudeModel) {
                    ForEach(ClaudeModel.allCases) { model in Text(model.title).tag(model) }
                }
                SettingsInlinePicker("Effort", selection: $settings.claudeEffort) {
                    ForEach(ClaudeReasoningEffort.allCases) { effort in Text(effort.title).tag(effort) }
                }
            }
            .padding(.leading, 48)
        case .cursor, .opencode, .antigravity, .appleIntelligence:
            EmptyView()
        case .ollama, .lmStudio, .openAI, .anthropic, .gemini, .openRouter, .groq:
            HStack(spacing: 8) {
                Text("Model")
                    .font(.system(size: 11, weight: .medium, design: .rounded))
                    .foregroundStyle(.white.opacity(0.48))
                TextField(provider.defaultModel, text: modelBinding(for: provider))
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 7, style: .continuous).stroke(.white.opacity(0.11), lineWidth: 1))
                if let options = viewModel.providerModelOptions[provider], !options.isEmpty {
                    Picker("Fetched models", selection: modelBinding(for: provider)) {
    ...
    private var providerPickerOptions: [ProviderSelection] {
        var options: [ProviderSelection] = [.auto]
        for provider in AssistantProvider.allCases where providerReady(provider) {
            options.append(provider.selection)
        }
        return options
    }

    private func providerReady(_ provider: AssistantProvider) -> Bool {
        health(for: provider)?.installed == true && health(for: provider)?.authenticated == true
    }

    private func providerStatus(_ provider: AssistantProvider) -> String {
        let health = health(for: provider)
        guard let health else { return "Checking" }
        if !health.installed { return provider.category == .cli ? "Missing" : "Unavailable" }
        if !health.authenticated {
            switch provider.category {
            case .cli: return "Needs login"
            case .local: return "Unavailable"
            case .byok: return "Not connected"
            }
        }
        return "Ready"
    }
    ...
    private func modelBinding(for provider: AssistantProvider) -> Binding<String> {
        ...
        case .codex, .claude, .cursor, .opencode, .antigravity, .appleIntelligence:
            return .constant(provider.defaultModel)
        }
    }
}
...
struct ChatHistorySettingsPage: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            SettingsPageHeader(title: "Chat History", subtitle: "Review and manage saved local conversations.")
            SettingsGroup(title: "Saved chats") {
                SettingsToggleRow(title: "Keep local history", detail: "Store prompts and answers in Application Support.", isOn: $settings.historyEnabled)
                HStack {
                    SettingsGlyph(systemName: "clock.arrow.circlepath", tone: .neutral)
                ...
                if visibleHistoryCount < history.items.count {
                    SettingsActionButton("Load more") {
...
struct SettingsBottomAction: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .foregroundStyle(.white.opacity(isDisabled ? 0.36 : 0.72))
            .frame(height: 27)
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
